...
维护者回归：使用 OCRmyPDF 环境的 Python 运行 `tests/test_red_boxes.py`，需要已有插件、pikepdf、Pillow 与 poppler。测试以插件真实文字层生成器创建临时小样，覆盖文本/图像保留、合法红图形、幂等、目录、只读预演及失败传播；它不替代 Apple Vision 识别链路的端到端测试。
