...
- 首次改动保存 `目标.pdf.bak-redbox`，再原子替换目标。旧备份与当前输入不同时另存 `.bak-redbox.<输入SHA256>`，支持同路径重跑 OCR，不覆盖历史备份；无框不写盘。恢复时选择对应输入的备份，先保存当前版本。
- 备份没有自动过期或删除策略，每份占用接近修复前 PDF 的体积。需要清理时，先按目标 PDF 的完整路径列举其 `.bak-redbox` 和 `.bak-redbox.<SHA256>` 历史备份，核对大小、内容及仍需保留的回滚版本，再由用户确认后删除指定文件；脚本不自动清理备份。
...
自动 OCR 路径同样保留备份。清理失败会保留当前产物并计为失败，不继续生成该文件的 Markdown；排查后可重新运行修复。

## 验证与升级

比较备份与修复件的 `pdftotext` 全文、页数及图像原始流哈希；在原问题页渲染足够分辨率的局部，确认红框消失且扫描内容保留。扫描图原始流可用 pikepdf `read_raw_bytes()` 核对。保存参数避免重压已有图像，但参数本身不替代产物比较。
...
维护者回归：使用 OCRmyPDF 环境的 Python 运行 `tests/test_red_boxes.py`，需要已有插件、pikepdf、Pillow 与 poppler。九项测试以插件真实文字层生成器创建临时小样，覆盖文本/图像保留、合法红图形及渲染差异、幂等、目录、只读预演、失败传播、连续重跑备份、普通表单/签名保护，以及 macOS 权限与 Finder 标签；它不替代 Apple Vision 识别链路的端到端测试。
